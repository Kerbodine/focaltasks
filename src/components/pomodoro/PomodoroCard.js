...
export default function PomodoroCard({ session }) {
  ...
  return (
    <li className="flex w-full items-center rounded-lg border-2 border-gray-100 px-1.5 py-1 dark:border-gray-800">
      <div className="mr-1 text-2xl">
        {session.completed ? (
          <HiCheckCircle className="text-accent" />
        ) : currentId === session.id ? (
          <HiMinusCircle className="animate-spin-slow text-gray-400 dark:text-gray-600" />
        ) : (
      ...
      <span className="ml-auto rounded-md bg-gray-100 px-1.5 py-0.5 text-sm font-medium text-gray-500 dark:bg-gray-800">
        {session.completedAt
